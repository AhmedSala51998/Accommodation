...
<head>
    ...
    <style>
        ...
        .excel-modal { display: none; position: fixed; z-index: 9999; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.6); backdrop-filter: blur(5px); }
        .excel-modal-content { background: white; margin: 3% auto; padding: 25px; border-radius: 24px; width: 95%; max-width: 1100px; box-shadow: 0 25px 50px rgba(0,0,0,0.3); animation: modalFadeIn 0.3s ease; }
        @keyframes modalFadeIn { from { opacity: 0; transform: translateY(-20px); } to { opacity: 1; transform: translateY(0); } }
        .excel-upload-zone { border: 3px dashed #cbd5e1; border-radius: 20px; padding: 40px; text-align: center; background: #f8fafc; cursor: pointer; transition: 0.3s; display: flex; flex-direction: column; align-items: center; gap: 15px; }
        .excel-upload-zone:hover { border-color: #4361ee; background: #f0f7ff; }
        .excel-preview-table-wrapper { margin-top: 20px; border: 1px solid #e2e8f0; border-radius: 12px; overflow: auto; max-height: 250px; background: white; }
        .excel-preview-table { width: 100%; border-collapse: collapse; font-size: 11px; }
        .excel-preview-table th { background: #f1f5f9; padding: 10px; border: 1px solid #e2e8f0; position: sticky; top: 0; font-weight: 800; }
...
<body>
        ...
        <div class="excel-modal-content">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; border-bottom:1px solid #eee; padding-bottom:15px;">
                ...
                <div style="display:flex; gap:10px;">
                    <button class="btn btn-secondary" style="padding:5px 15px; font-size:12px; background:#e2e8f0;" onclick="excelImporter.downloadSample()">
            ...
            <div style="background:#f1f5f9; padding:15px; border-radius:12px; margin-bottom:20px; display:flex; justify-content:space-between; align-items:center;">
                <div style="font-weight:800; color:#4361ee;">هل تريد استيراد هذه البيانات إلى الأرشيف؟</div>
                <select id="excelImportTarget" style="padding:10px 20px; border-radius:10px; border:1px solid #ddd; outline:none; font-weight:800; cursor:pointer;">
                    <option value="0">لا (إلى الجدول الرئيسي)</option>
                    <option value="1">نعم (إلى الأرشيف مباشرة)</option>
                </select>
            </div>

            <div id="excelDropZone" class="excel-upload-zone" onclick="document.getElementById('excelFileInput').click()">
                ...
                <input type="file" id="excelFileInput" accept=".xlsx, .xls" style="display:none;" onchange="excelImporter.handleFile(this.files[0])">
            </div>

            <div id="excelPreviewSection" style="display:none; margin-top:20px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                    <p style="font-weight:800; color:#4361ee; margin:0;">معاينة البيانات (تم قراءة <span id="excelRowCount">0</span> سجل):</p>
                    <button id="showAllPreviewBtn" class="btn btn-secondary" style="display:none; padding:5px 15px; font-size:12px;" onclick="excelImporter.showFullPreview()">عرض كل السجلات</button>
                </div>
            ...
            <div style="margin-top:25px; border-top:1px solid #eee; padding-top:20px; display:flex; justify-content:flex-end; gap:10px;">
                ...
                <button id="confirmExcelImportBtn" class="btn btn-primary" disabled onclick="excelImporter.startImport()">تأكيد وحفظ البيانات</button>
            </div>
        </div>
    </div>
    ...
    <div id="addModal" class="modal">
        <div class="modal-content large">
            <div class="modal-header"><h2>إضافة عاملات (نظام الصفين الذكي)</h2><span class="close">&times;</span></div>
            <div class="modal-body"><div id="addBody" class="worker-group-list"></div><div class="modal-actions"><button id="addRowBtn" class="btn btn-secondary">إضافة عاملة جديدة</button></div></div>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content large">
            <div class="modal-header"><h2>تعديل بيانات العاملات المختارة</h2><span class="close">&times;</span></div>
            <div class="modal-body"><div id="editBody" class="worker-group-list"></div></div>
    ...
    <div id="officesModal" class="modal">
        <div class="modal-content large" style="max-width: 1000px;">
            <div class="modal-header"><h2><i class="fas fa-building"></i> إدارة المكاتب</h2><span class="close">&times;</span></div>
            <div class="modal-body">
                <div class="header-actions" style="margin-bottom: 30px; justify-content: center; padding: 20px; background: #f8fafc; border-radius: 16px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px; margin-right: 20px; padding-right: 20px; border-right: 2px solid #e2e8f0;">
                        <input type="checkbox" id="selectAllOffices" style="width: 18px; height: 18px; cursor: pointer;">
    ...
    <div id="addOfficesModal" class="modal">
        <div class="modal-content">
            <div class="modal-header"><h2 id="officeModalTitle">إضافة مكاتب جديدة</h2><span class="close">&times;</span></div>
            <div class="modal-body"><div id="officeInputsContainer" class="worker-group-list"></div><div class="modal-actions"><button id="addOfficeInputBtn" class="btn btn-secondary">إضافة حقل مكتب آخر</button></div></div>
    ...
    <div id="detailModal" class="modal">
        <div class="modal-content detail-modal-content">
            <div class="modal-header"><h2 id="detailTitle">تفاصيل</h2><span class="close" onclick="document.getElementById('detailModal').style.display='none'">&times;</span></div>
            <div class="modal-body"><p id="detailContent"></p></div>
